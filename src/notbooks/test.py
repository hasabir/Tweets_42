...
# def train_word2vec(processed_tweets, embedding_size=5144, window_size=5, num_negative_samples=5, learning_rate=0.1, epochs=10):
#     vocab = set(word for tweet in processed_tweets for word in tweet)  # Get unique vocabulary
#     word_to_idx = {word: idx for idx, word in enumerate(vocab)}  # Word to index mapping
...
#                     if center_idx == context_idx:  # Skip center word itself
#                         continue
#                     context_word_idx = word_to_idx[tweet[context_idx]]
#                     # Yield positive pair
#                     yield center_word_idx, context_word_idx, 1
#                     # Yield negative samples
#                     for _ in range(num_negative_samples):
#                         negative_word_idx = np.random.randint(0, vocab_size)
#                         while negative_word_idx == center_word_idx or negative_word_idx == context_word_idx:
#                             negative_word_idx = np.random.randint(0, vocab_size)
#                         yield center_word_idx, negative_word_idx, 0
    
#     def train_embeddings():
#         for epoch in range(epochs):
#             for center_idx, context_idx, label in generate_training_data():
#                 center_vec = main_embeddings[center_idx]
#                 context_vec = context_embeddings[context_idx]
#                 dot_product = np.dot(center_vec, context_vec)
#                 prediction = sigmoid(dot_product)
#                 error = label - prediction
#                 grad = error * learning_rate
#                 main_embeddings[center_idx] += grad * context_vec
...
#     tweet_embeddings = []
#     for tweet in processed_tweets:
#         tweet_indices = [word_to_idx[word] for word in tweet if word in word_to_idx]
#         if tweet_indices:
#             tweet_embedding = main_embeddings[tweet_indices].mean(axis=0)
#         else:
